...
def action_adjusting(direction, steer, acc, brake, speed, episode_name, before_steering):
...
def action_adjusting_town02(direction, steer, acc, brake, speed, episode_name, before_steering):
    weather, exp_id, se_point = saperate_environment(episode_name)
    ...
    if direction == 0:
        ...
        if weather == "14":
            ...
            if speed > curve_limit_speed_1_2:
                ...
                brake = 1
            if exp_id == "3":
                steer = steering_multi_function(speed, 1.42) * steer
            else:
                steer = steering_multi_function(speed, 1.5) * steer
        if steer > 0:
            steer = 0
