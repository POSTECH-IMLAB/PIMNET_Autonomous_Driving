...
def action_adjusting(direction, steer, acc, brake, speed, pred_speed, episode_name, loc_x, loc_y, running_cnt, stopping_cnt):
    ...
    if direction == 0:
        ...
        if weather == '14' and int(loc_x) in (351, 352, 353, 354, 356, 357) and int(loc_y) in (331, 332, 333) and steer > 0.1:
...
def action_adjusting_carla100(direction, steer, acc, brake, speed, pred_speed, episode_name, loc_x, loc_y, running_cnt, stopping_cnt):
...
def action_adjusting_town02(direction, steer, acc, brake, speed, pred_speed, episode_name, loc_x, loc_y, running_cnt, stopping_cnt):
    weather, exp_id, se_point = saperate_environment(episode_name)
    ori_acc = acc
    ori_brake = brake
    brake = brake / 1.5
    ...
    ''' pred_speed를 이용해 adj '''
    if pred_speed > 30 and speed < 30:
        acc = np.fmax(acc, 0.721)
        brake = 0
    elif pred_speed > 20 and speed < 20:
        acc = np.fmax(acc, 0.621)
        brake = 0
    elif pred_speed > 10 and speed < 10:
        acc = np.fmax(acc, 0.521)
        brake = 0
    elif speed < 3 and pred_speed > 3:
        acc = np.fmax(acc, 0.421)
        brake = 0


    # if (pred_speed < speed / 2 and speed > 5) or (pred_speed < 0.4 and brake > 0.01):
    if ori_acc < 0.05 and ori_brake > 0.004 and pred_speed < 0.39:
        acc = 0
        brake = np.fmax(0.711, ori_brake)
    elif speed - pred_speed > 15:
        acc = 0
        brake = np.fmax(0.651, ori_brake)
    elif speed - pred_speed > 12:
        acc = 0
        brake = np.fmax(0.611, ori_brake)
    elif pred_speed < 0.39 and speed > 8:
        acc = 0
        brake = np.fmax(0.699, ori_brake)

    if speed > 32:
        acc = 0

    if ori_brake > 0.4 and pred_speed < 30 and speed > 10:
        acc = 0
        brake = ori_brake
    elif ori_brake > 0.6 and speed > 5:
        acc = 0
        brake = ori_brake
    ...
    if int(brake) > int(acc):
        ...
        if speed < 3 and pred_speed > speed:
            acc = np.fmax(np.fmin(acc, 1.0), 0.53)
        elif speed < 7 and pred_speed > speed:
            acc = np.fmax(np.fmin(acc, 1.0), 0.59)
