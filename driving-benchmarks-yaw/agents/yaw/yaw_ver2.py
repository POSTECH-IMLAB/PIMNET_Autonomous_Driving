...
import agents.yaw.modules.yaw_net_v2 as resnet_carla
...
class ImitationLearning(Agent):
    ...
    def _control_function(self, image_input, speed, control_input):
        ...
        with torch.no_grad():
            ...
            pred_speed, pred_posi_x, pred_posi_y, pred_steering,  \
            rela_pred_speed, rela_pred_posi_x, rela_pred_posi_y, rela_pred_steering = self.model(img_ts, speed_ts)
        ...
        rela_pred_posi_x = rela_pred_posi_x[0][control_input].cpu().numpy()
        rela_pred_posi_y = rela_pred_posi_y[0][control_input].cpu().numpy()
        rela_pred_steering = rela_pred_steering[0][control_input].cpu().numpy()
        rela_predicted_speed = rela_pred_speed.squeeze().item()

        print('x: ', pred_posi_x ,':',rela_pred_posi_x, ' y: ', pred_posi_y ,':',rela_pred_posi_y, ' st: ', pred_steering ,':',rela_pred_steering
              , ' sp: ', predicted_speed ,':',rela_predicted_speed)

        predicted_speed = (rela_predicted_speed + predicted_speed) / 2
        pred_steering = rela_pred_steering
