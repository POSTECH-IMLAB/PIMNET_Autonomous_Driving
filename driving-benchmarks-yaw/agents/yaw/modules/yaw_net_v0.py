...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.action_lateral = nn.ModuleList([
            nn.Sequential(
                nn.Linear(512, 256),
                nn.Dropout(0.5),
                nn.ReLU(),
    ...
    def forward(self, x, speed):
        ...
        x_posi = self.fc_posi(x_posi)

        speed = self.speed_fc(speed)
        emb = torch.cat([x_posi, speed], dim=1)
        emb = self.emb_fc_posi(emb)
        pred_posi_x = torch.cat([out(emb) for out in self.fc_posi_x], dim=1)
        pred_posi_y = torch.cat([out(emb) for out in self.fc_posi_y], dim=1)

        ''' speed pred '''
        pred_speed = self.speed_branch(x_posi)
        ''' for lateral control '''
        x = self.layer3(x)
        x = self.layer4(x)
        x = x.view(x.size(0), -1)
        x_steering = self.fc_steering(x)
        emb = torch.cat([x_steering, speed], dim=1)
        emb = self.emb_fc(emb)

        pred_steering = torch.cat([out(emb) for out in self.action_lateral], dim=1)
