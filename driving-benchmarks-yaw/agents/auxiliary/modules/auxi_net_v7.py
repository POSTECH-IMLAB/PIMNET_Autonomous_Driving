...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        #self.layer3_4 = self._make_layer_4(block, 256, layers[2], stride=2)
        ...
        ''' Relation '''
        ''' adaptor '''
        #self.adaptor_longi_layer3 = nn.Sequential(
        #    nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)

        #self.adaptor_late_layer3 = nn.Sequential(
        #    nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)

        #self.adaptor_posi_layer3 = nn.Sequential(
        #    nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)

        #self.adaptor_yaw_layer3 = nn.Sequential(
        #    nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)
        ...
        #self.aggregator_layer3_1 = nn.Sequential(
        #    nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)

        #self.aggregator_layer3_2 = nn.Sequential(
        #    nn.Conv2d(1024, 512, kernel_size=3, stride=1, padding=1, bias=False),
        #    nn.BatchNorm2d(512),
        #    nn.ReLU(),
        #)
    ...
    def forward(self, x, speed):
        ...
        ''' for longitudial '''
        x_longi = self.layer3(x)
        #x_longi_adp3 = self.adaptor_longi_layer3(x_longi)
        x_longi = self.layer4(x_longi)
        #x_longi_adp4 = self.adaptor_longi_layer4(x_longi)
        ...
        ''' for lateral '''
        x_late = self.layer3_2(x)
        #x_late_adp3 = self.adaptor_late_layer3(x_late)
        x_late = self.layer4_2(x_late)
        #x_late_adp4 = self.adaptor_late_layer4(x_late)
        ...
        #x_posi_adp3 = self.adaptor_posi_layer3(x_posi)
        #x_posi = self.layer4_3(x_posi)
        #x_posi_adp4 = self.adaptor_posi_layer4(x_posi)
        #x_posi = x_posi.view(x_posi.size(0), -1)
        #x_posi = self.fc_specialize_3(x_posi)
        #pred_posi = self.posi_branch(x_posi)

        ''' for yaw '''
        #x_yaw = self.layer3_4(x)
        #x_yaw_adp3 = self.adaptor_yaw_layer3(x_yaw)
        #x_yaw = self.layer4_4(x_yaw)
        #x_yaw_adp4 = self.adaptor_yaw_layer4(x_yaw)
        #x_yaw = x_yaw.view(x_yaw.size(0), -1)
        #x_yaw = self.fc_specialize_4(x_yaw)
        #pred_yaw = self.yaw_branch(x_yaw)


        #x_longi_yaw = torch.cat((x_longi_adp3, x_yaw_adp3), 1)
        #x_longi_yaw = self.aggregator_layer3_1(x_longi_yaw)
        #x_longi_yaw = torch.cat((x_longi_yaw, x_longi_adp4, x_yaw_adp4), 1)
        ...
        #x_late_posi = torch.cat((x_posi_adp3, x_late_adp3), 1)
        #x_late_posi = self.aggregator_layer3_2(x_late_posi)
        #x_late_posi = torch.cat((x_late_posi, x_posi_adp4, x_late_adp4), 1)
...
def resnet34_carla(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_state_dict_layer3_2 = {k.replace("layer3", "layer3_2"): v for k, v in pretrained_state_dict.items()
                                          if k.find("layer3") >= 0 and k.replace("layer3", "layer3_2") in now_state_dict}
        pretrained_state_dict_layer3_3 = {k.replace("layer3", "layer3_3"): v for k, v in pretrained_state_dict.items()
                                          if k.find("layer3") >= 0 and k.replace("layer3", "layer3_3") in now_state_dict}
        pretrained_state_dict_layer3_4 = {k.replace("layer3", "layer3_4"): v for k, v in pretrained_state_dict.items()
                                          if k.find("layer3") >= 0 and k.replace("layer3", "layer3_4") in now_state_dict}
        ...
        now_state_dict.update(pretrained_state_dict)
        now_state_dict.update(pretrained_state_dict_layer3_2)
        now_state_dict.update(pretrained_state_dict_layer3_3)
        now_state_dict.update(pretrained_state_dict_layer3_4)
