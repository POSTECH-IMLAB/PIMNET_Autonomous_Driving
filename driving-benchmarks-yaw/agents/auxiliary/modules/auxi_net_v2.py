...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        # self.posi_branch = nn.ModuleList([
        #     nn.Sequential(
        #         nn.Linear(512, 256),
        #         nn.ReLU(),
        #         nn.Dropout(0.3),
        ...
        # ''' adaptor '''
        # self.adaptor_speed_layer3 = nn.Sequential(
        #     nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(),
        # )
        # self.adaptor_speed_layer4 = nn.Sequential(
        #     nn.Conv2d(512, 512, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        #
        # self.adaptor_posi_1_layer3 = nn.Sequential(
        #     nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(),
        # )
        # self.adaptor_posi_1_layer4 = nn.Sequential(
        #     nn.Conv2d(512, 512, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        #
        # self.adaptor_posi_2_layer3 = nn.Sequential(
        #     nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(),
        # )
        # self.adaptor_posi_2_layer4 = nn.Sequential(
        #     nn.Conv2d(512, 512, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        #
        # self.adaptor_steer_layer3 = nn.Sequential(
        #     nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(256),
        #     nn.ReLU(),
        # )
        # self.adaptor_steer_layer4 = nn.Sequential(
        #     nn.Conv2d(512, 512, kernel_size=1, stride=1, bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        #
        # ''' aggregator '''
        # self.aggre_speed_posi = nn.Sequential(
        #     nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=(1,1), bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        # self.aggre_steering_posi = nn.Sequential(
        #     nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=(1,1), bias=False),
        #     nn.BatchNorm2d(512),
        #     nn.ReLU(),
        # )
        ...
        # )
        #
        # self.emb_adaptor_speed = nn.Sequential(
        #     nn.Linear(512 + 128, 512),
        #     nn.ReLU(),
        #     nn.Dropout(0.5),
        # )
        ...
        # )
        #
        # self.rela_steering_branch = nn.ModuleList([
        #     nn.Sequential(
        #         nn.Linear(512, 256),
        #         nn.ReLU(),
        #         nn.Dropout(0.3),
    ...
    def forward(self, x, speed):
        ...
        # x_speed_adp1 = self.adaptor_speed_layer3(x_speed)
        x_speed = self.layer4(x_speed)
        # x_speed_adp2 = self.adaptor_speed_layer4(x_speed)
        x_speed = x_speed.view(x_speed.size(0), -1)
        x_speed = self.fc_specialize_1(x_speed)
        pred_speed = self.speed_branch(x_speed)

        ''' for posi '''
        # x_posi = self.layer3_2(x)
        # x_posi_adp1_1 = self.adaptor_posi_1_layer3(x_posi)
        # x_posi_adp1_2 = self.adaptor_posi_2_layer3(x_posi)
        # x_posi = self.layer4_2(x_posi)
        # x_posi_adp2_1 = self.adaptor_posi_1_layer4(x_posi)
        # x_posi_adp2_2 = self.adaptor_posi_2_layer4(x_posi)
        # x_posi = x_posi.view(x_posi.size(0), -1)
        # x_posi = self.fc_specialize_2(x_posi)
        #
        # emb_posi = torch.cat([x_posi, ipt_speed], dim=1)
        # emb_posi = self.emb_fc_posi(emb_posi)
        # pred_posi = torch.cat([out(emb_posi) for out in self.posi_branch], dim=1)

        ''' for steering '''
        x_steering = self.layer3_3(x)
        # x_steering_adp1 = self.adaptor_steer_layer3(x_steering)
        x_steering = self.layer4_3(x_steering)
        # x_steering_adp2 = self.adaptor_speed_layer4(x_steering)
        x_steering = x_steering.view(x_steering.size(0), -1)
        x_steering = self.fc_specialize_3(x_steering)
        ...
        ''' for relation '''
        # x_speed_posi = self.aggre_speed_posi(x_speed_adp1 + x_posi_adp1_1)
        # x_speed_posi = x_speed_posi + x_speed_adp2 + x_posi_adp2_1
        # x_speed_posi = x_speed_posi.view(x_speed_posi.size(0), -1)
        # x_speed_posi = self.fc_specialize_adap_1(x_speed_posi)
        # x_speed_posi = torch.cat([x_speed_posi, ipt_speed], dim=1)
        # x_speed_posi = self.emb_adaptor_speed(x_speed_posi)
        # rela_pred_steering = torch.cat([out(x_speed_posi) for out in self.rela_steering_branch], dim=1)
        #
        # x_steer_posi = self.aggre_steering_posi(x_steering_adp1 + x_posi_adp1_2)
        # x_steer_posi = x_steer_posi + x_steering_adp2 + x_posi_adp2_2
        # x_steer_posi = x_steer_posi.view(x_steer_posi.size(0), -1)
        # x_steer_posi = self.fc_specialize_adap_2(x_steer_posi)
        # rela_pred_speed = self.rela_speed_branch(x_steer_posi)

        return pred_speed, pred_steering
