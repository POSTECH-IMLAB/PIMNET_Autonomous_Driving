...
import agents.auxiliary.modules.auxi_net_v7 as resnet_carla

from version084.benchmark_tools.agent import Agent
from version084.carla.carla_server_pb2 import Control

from utils.auxi_ver7_adj import action_adjusting, action_adjusting_town02_carla100
...
class ImitationLearning(Agent):
    ...
    def _compute_action(self, rgb_image, measurements, episode_name, direction=None):
        ...
        if self.before_direction == 1 and direction == 2:
            ...
            direction = 0
        elif self.before_direction == 3 and direction == 2:
            direction = 0
        elif self.before_direction == 3 and direction == 1:
            direction = 0
        else:
        ...
        ori_brake = brake
        ori_acc = acc
        #steer, acc, brake = action_adjusting_carla100(direction, steer, acc, brake, speed, pred_speed, episode_name, loc_x,
        #                                     loc_y, self.running_cnt, self.stopping_cnt)
        steer, acc, brake = action_adjusting_town02_carla100(direction, steer, acc, brake, speed, pred_speed, episode_name, loc_x, loc_y, self.running_cnt, self.stopping_cnt)
    ...
    def _control_function(self, image_input, speed, control_input):
        ...
        real_predicted = predicted_speed * 40.0

        return predicted_steers, predicted_acc, predicted_brake, real_predicted
